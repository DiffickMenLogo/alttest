...
function breakWheel() {
    ...
    for (const prop of ExcludeCollisionProps){
        if (natives.hasClosestObjectOfTypeBeenBroken(veh.pos.x, veh.pos.y, veh.pos.z, 5, prop, 0) || natives.getClosestObjectOfType(veh.pos.x, veh.pos.y, veh.pos.z, 3, prop, false, false, false)) {
            obj = true;
            break;
        }
    }
...
function checkVehicleDamage(vehicle) {
    ...
    interval = alt.setInterval(()=>{
        ...
        if ((Math.abs(speedBuffer[0] - speedBuffer[1]) > 20 || speed > 120) && natives.hasEntityCollidedWithAnything(vehicle)) {
            // shofter collision
            breakWheel();
