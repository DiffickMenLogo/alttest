...
const startDeath = ()=>{
    ...
    if (!native.isEntityInWater(LOCAL_PLAYER)) {
        shuffleArray(BLOOD_DAMAGE_PACKS).slice(0, 3).forEach((e)=>native.applyPedDamagePack(LOCAL_PLAYER, e, 100.0, 1.0));
    }
    deathTick = new alt.Utils.EveryTick(()=>{
        ...
        if (LOCAL_PLAYER.vehicle) native.taskLeaveAnyVehicle(LOCAL_PLAYER, 0, 256) // don't close the vehicle door
        ;
    });
