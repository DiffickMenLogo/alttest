...
function create_noclip_camera() {
    ...
    noclip_camera = native.createCamWithParams(noclip_camera_type, alt.Player.local.pos.x, alt.Player.local.pos.y, alt.Player.local.pos.z, gameplayCamRot.x, gameplayCamRot.y, gameplayCamRot.z, native.getGameplayCamFov(), false, 2);
    native.setCamActiveWithInterp(noclip_camera, native.getRenderingCam(), 250, 0, 0);
...
function set_camera_rotation(camera_rotation) {
    ...
    native.setEntityRotation(alt.Player.local.scriptID, rotation.x, 0, rotation.z, 2, true);
}
...
function set_camera_position(camera_position, camera_rotation) {
    const current_multiplier = native.isDisabledControlPressed(0, 21) ? baseSpeedMultiplier * 1.75 : baseSpeedMultiplier;
    const offset = get_next_offset(camera_rotation).mul(current_multiplier * 0.25);
    const new_position = offset.add(camera_position);
    native.setCamCoord(noclip_camera, new_position.x, new_position.y, new_position.z);
    native.setEntityCoordsNoOffset(alt.Player.local.scriptID, new_position.x, new_position.y, new_position.z, false, false, false);
}
...
function get_next_offset(camera_rotation) {
    ...
    if (native.isDisabledControlPressed(0, 32)) {
        const forward = forward_vector_from_quaternion(euler_angles_to_quaternion(camera_rotation));
        offset = offset.add(forward);
    }
    // Left (A)
    if (native.isDisabledControlPressed(0, 34)) {
        const left = forward_vector_from_quaternion(euler_angles_to_quaternion(new alt.Vector3(0, 0, camera_rotation.z - 270)));
        offset = offset.add(left);
    }
    // Backward (S)
    if (native.isDisabledControlPressed(0, 33)) {
        let backwards = forward_vector_from_quaternion(euler_angles_to_quaternion(new alt.Vector3(camera_rotation.x, camera_rotation.y, camera_rotation.z))).mul(-1);
        offset = offset.add(backwards);
    }
    // Right (D)
    if (native.isDisabledControlPressed(0, 35)) {
        let right = forward_vector_from_quaternion(euler_angles_to_quaternion(new alt.Vector3(0, 0, camera_rotation.z - 90)));
...
function euler_angles_to_quaternion(euler_angles) {
    ...
    const x = Math.sin(roll / 2) * Math.cos(pitch / 2) * Math.cos(yaw / 2) - Math.cos(roll / 2) * Math.sin(pitch / 2) * Math.sin(yaw / 2);
    const y = Math.cos(roll / 2) * Math.sin(pitch / 2) * Math.cos(yaw / 2) + Math.sin(roll / 2) * Math.cos(pitch / 2) * Math.sin(yaw / 2);
    const z = Math.cos(roll / 2) * Math.cos(pitch / 2) * Math.sin(yaw / 2) - Math.sin(roll / 2) * Math.sin(pitch / 2) * Math.cos(yaw / 2);
    const w = Math.cos(roll / 2) * Math.cos(pitch / 2) * Math.cos(yaw / 2) + Math.sin(roll / 2) * Math.sin(pitch / 2) * Math.sin(yaw / 2);
