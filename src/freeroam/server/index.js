...
alt.on("playerConnect", (player) => {
  ...
  let connectTimeout = alt.setTimeout(() => {
    if (player && player.valid) {
      ...
      chat.broadcast(
        `{1cacd4}${player.name} {ffffff}has {00ff00}joined {ffffff}the Server..  (${playerCount} players online)`
      );
      chat.send(
        player,
        "{80eb34}Press {34dfeb}T {80eb34}and type {34dfeb}/help {80eb34}to see all available commands.."
      );
    }
...
alt.on("playerDeath", (player, killer, weapon) => {
  ...
  if (killer) {
    ...
    SendNotificationToAllPlayer(
      `~r~<C>${killer.name}</C> ~s~killed ~b~<C>${player.name}</C>`
    );
  } else {
...
function SendNotificationToAllPlayer(
  message,
  textColor = 0,
  bgColor = 2,
  blink = false
...
alt.on("playerDisconnect", (player, reason) => {
  const playerCount = alt.Player.all.length;
  chat.broadcast(
    `{1cacd4}${player.name} {ffffff}has {ff0000}left {ffffff}the Server.. (${playerCount} players online)`
  );
...
chat.registerCmd("help", (player, args) => {
  ...
  chat.send(
    player,
    "{ff0000}= {34abeb}/veh {40eb34}(model)   {ffffff} Spawn a Vehicle"
  );
  chat.send(
    player,
...
chat.registerCmd("hp", (player, args) => {
  if (args.length === 0) {
    chat.send(player, "Usage: /hp (health)");
    return;
  }
...
chat.registerCmd("armour", (player, args) => {
  if (args.length === 0) {
    chat.send(player, "Usage: /armour (armour)");
    return;
  }
  player.armour = args[0];
});

chat.registerCmd("checkPos", (player, args) => {
  alt.log(`Position: ${player.pos.x}, ${player.pos.y}, ${player.pos.z}`);
  chat.send(
    player,
    `Position: ${player.pos.x}, ${player.pos.y}, ${player.pos.z}`
